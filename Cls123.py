import cv2
import pandas
import numpy as np  
from sklearn.datasets import fetch_openml
from sklearn.model_selection import train_test_split
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import accuracy_score

# Python Image Library (not  Public Interest Litigation)
from PIL import Image
import PIL.ImageOps
import os 

# Secure Soket Layer {used in websites to secure it from hackers}
import ssl

# If one of the condition or both of them is false , then ssl will create a default https context {unverified context} to bypass the ssl network
if(not os.environ.get('PYTHONHTTPSVERIFY','') and getattr(ssl,'_create_unverified_context' , None)) : 
    # context is a setting used for creating a secured connection with the website.
    ssl._create_default_https_context = ssl._create_unverified_context

X,y = fetch_openml('mnist_784' , version = 1 , return_X_y = True)

X_train,X_test,y_train,y_test = train_test_split(X, y , random_state = 9 , train_size = 7500 , test_size = 2500)

X_train = X_train/255
X_test = X_test/255

clf = LogisticRegression(solver = 'Saga' , multi_class = 'multinomial').fit(X_train , y_train)
y_predict = clf.predict(X_test)

accuracy = accuracy_score(y_test , y_predict)
print('The Accuracy of the Classifier is :- {}'.format(accuracy))

cap =  cv2.VideoCapture(0)
while(True) : 
    # Accept block is used to catch the exceptions/errors that might arise while executing the code in the try block.
    try : 
        ret,frame = cap.read()
        # To convert the colorful image to black and white image
        gray = cv2.cvtColor(frame , cv2.COLOR_BGR2GRAY)
        # Determining the no. of rows and no of columns
        #Attribute :- Property of the class
        height,width = gray.shape

        upper_left = (int((width/2) - 56) , int((height/2) - 56))
        bottom_right = (int((width/2) + 56) , int((height/2) + 56))

        # To draw a rectangle , CO-ORDINATES , random_color , width = 2
        cv2.rectangle(gray  , upper_left , bottom_right , (0,255,0) , 2)
        
        # Region of Interest ; Syntax => gray(change in y , change  in x)
        roi = gray[upper_left[1] : bottom_right[1] , upper_left[0] : bottom_right[0]]

        im_pil = Image.fromarray(roi)

        # To convert it into a gray scale ; L for gray scale
        image_bw = im_pil.convert('L') 
        # Resizing and making the image smoother with Image.ANTIALIAS
        image_bw_resized = image_bw.resize((28,28) , Image.ANTIALIAS)

        # Inverting the image
        image_bw_resized_inverted = PIL.ImageOps.invert(image_bw_resized)

        # To specify the lower range 
        pixel_filter = 20 

        #  Removing the lighter pixel {which are below 20 percentile from the max_pixel} from the image captured roi
        min_pixel = np.percentile(image_bw_resized_inverted, pixel_filter) 
        image_bw_resized_inverted_scaled = np.clip(image_bw_resized_inverted-min_pixel, 0, 255) 

        #  Determining the brightess pixel
        max_pixel = np.max(image_bw_resized_inverted) 

        #  Scaling ; Dividing the remaining the individual values by the max to determine 0 and 1 for images
        image_bw_resized_inverted_scaled = np.asarray(image_bw_resized_inverted_scaled)/max_pixel 

        test_sample = np.array(image_bw_resized_inverted_scaled).reshape(1,784)
        # Data obtained from the area of interest and reshaped it into 784 grids of pixels with 0 and 1 value 
        # We converted it in the format as of that we trained the classifier by reshaping it.


        prediction = clf.predict(test_sample)

        print('Prediction class is :- {}'.format(prediction))

        cv2.imshow('frame' , gray)
        
        # waitKey will give value -1 {not pressed something} and 32 byte integer {when a key is pressed}
        # ord('q') gives the ascii value of q ; gives only 8 byte , then 0xFF is used to discard/trunket the first 24 byte and will do a bitwise operation
        # and this will be converted into 8 byte 

        # So now, it will have the same bytes value 
        if cv2.waitKey(1) & 0xFF == ord('q') : 
            break
        
    except Exception as e : 
        pass

cap.release()
cv2.destroyAllWindows()